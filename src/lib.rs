...
use split_shenanigans::{TupleMutIntoNonNull, TupleImmutIntoNonNull, TupleNonNullIntoGenRef};
...
impl<'s, M: Mutability, T: ?Sized> GenRef<'s, M, T> {
            ...
            FIM: FnOnce(&T) -> (&U, &V)
    {
        unsafe {
            //TODO: Add safety comment
            TupleNonNullIntoGenRef::into_genref(
                M::split(self.ptr, (), |t, ()| fn_mut(t), |t, ()| fn_immut(t))
            )
        }
    }
            ...
            FIM: FnOnce(&T,     X) -> (&U, &V)
    {
        unsafe {
            //TODO: Add safety comment
            TupleNonNullIntoGenRef::into_genref(
                M::split(self.ptr, moved, fn_mut, fn_immut)
            )
        }
    }
}
